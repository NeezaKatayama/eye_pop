...
function buildSummary(fatigue, dryEye, focus, recovery) {
  if (fatigue > 0.68) {
    return "まぶた閉鎖時間が長めで、眠気または疲労サインが強めです。短い休憩や視線リセットを挟むと良さそうです。";
  }
  if (dryEye > 0.62) {
    return "瞬き不足または部分瞬きが増えており、ドライアイ寄りのパターンです。画面から目を離して意識的に完全瞬きを入れるのが向いています。";
  }
  if (focus > 0.68 && recovery > 0.58) {
    return "視線安定性は高く、比較的良いコンディションです。今の照明と姿勢を維持しつつ、長時間連続使用だけ避けると良さそうです。";
  }
  return "大きな負荷は見えませんが、まだ観測時間が短い可能性があります。60秒ほど継続するとスコアが安定します。";
}
